...
- analyze-compaction.sh: Queries MeiliSearch for PreCompact and PreToolUse events, measures post-compaction "re-read penalty" (exploration calls in a time window after each compaction). Compares two sessions. Args: [session_a] [session_b] or auto-detects. Env: MEILI_URL, COMPACT_WINDOW (default: 300s).
